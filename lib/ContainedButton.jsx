...
const styles = (theme) => ({
    ...
    primary: {
        background: makeBackground(theme.palette.primary.main, theme.palette.primary.light),
        boxShadow: makeBoxShadow(theme.palette.primary.light)
    },
    secondary: {
        background: makeBackground(theme.palette.secondary.main, theme.palette.secondary.light),
...
const ContainedButton = ({ classes, color, children, ...others }) => {
    ...
    return (<MuiButton {...others} className={classNames}>
      {children}
    </MuiButton>);
};
